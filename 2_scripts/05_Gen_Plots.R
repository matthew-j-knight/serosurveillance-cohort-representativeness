...
# 2. Generate heatmaps
# 3. Create figures 2 and 3
# 4. Additional analyses: AARB and Duncan Dissimilarity Index
# 5. Sensitivity analysis 1
# 6. Sensitivity analysis 2
# 7. Sensitivity analysis 3
# 8. Sensitivity analysis 4
# 9. Plot bootstrap distributions
...
apl_asu1_qab<-read.csv("./1_data/private/apl_asu_qab.csv")
apl_sqm1_qab<-read.csv("./1_data/private/apl_sqm_qab.csv")
apl_sqs1_qab<-read.csv("./1_data/private/apl_sqs_qab.csv")
...
abc_asu1_qab<-read.csv("./1_data/private/abc_asu_final_qab.csv")
abc_asr1_qab<-read.csv("./1_data/private/abc_asr_final_qab.csv")
...
clsa_asu1_qab<-read.csv("./1_data/private/clsa_asu_final_qab.csv")
clsa_asr1_qab<-read.csv("./1_data/private/clsa_asr_final_qab.csv")
clsa_sqm1_qab<-read.csv("./1_data/private/clsa_sqm_final_qab.csv")
clsa_sqs1_qab<-read.csv("./1_data/private/clsa_sqs_final_qab.csv")
...
can_asu1_s4 <- read.csv("./1_data/private/can_asu_finals4.csv")
can_asr1_s4 <- read.csv("./1_data/private/can_asr_finals4.csv")
...
# Read in results of AARB and Duncan Dissimilarity Index analyses
ddi_df <- read.csv("./1_data/private/ddi_analysis.csv")
aarb_df <- read.csv("./1_data/private/aarb_analysis.csv")
...
#ggsave("4_output/figs/supp_asu.pdf",width=12.5,height=8.5,unit="in")
...
#ggsave("4_output/figs/supp_asr.pdf",width=12.5,height=8.5,unit="in")
...
supp_qm<-ggplot(all_dtsqm,aes(x = sex, y = factor(quintmat,
  ...
  labs(fill = "Count",
       y = "neighborhood material deprivation quintile",
       x = "Sex")+
  theme(legend.text = element_text(size = 6.5),
        ...
        axis.text.y = element_text(size = 8.5,margin = margin(r = 1.5)))+
  guides(fill = guide_legend(byrow = T))

#ggsave("4_output/figs/supp_sqm.pdf",width=5.0,height=4.0,unit="in")
...
supp_sqs<-ggplot(all_dtsqs,aes(x = sex, y = factor(quintsoc,
  ...
  geom_text(aes(label = scales::number(count,big.mark = ",")),color = "black",size = 2.5)+
  labs(fill = "Count",
       y = "neighborhood social deprivation quintile",
       x = "Sex")+
  theme(legend.text = element_text(size = 6.5),
        legend.position = "right",
        legend.margin = margin(rep(0,4)),
        ...
        axis.title = element_blank(),
        legend.title = element_text(size = 8.5),
        axis.text.y = element_text(size = 8.0, margin = margin(r = 1.5))
        )+
  guides(fill = guide_legend(byrow = T))

#ggsave("4_output/figs/supp_sqs.pdf",width=5.0,height=4.0,unit="in")
...
cols <- c("#ec475a","#f17a88","#85BC47", "#65b6d0", "#0087c3")
...
f1_p<-ggplot(f1,aes(x = sex,y = age_groups,
  ...
  theme(axis.title = element_blank(),
        ...
        legend.margin = margin(t = 0.1,l = 0.1,r = 1.5))

f1_p<-cowplot::plot_grid(f1_p,scale = 0.75)

#ggsave("4_output/figs/f1.pdf",width=10.75,height=7.5,unit="in")
...
#ggsave("4_output/figs/s1_sqs.pdf",width=7.0,height=4.0,unit="in")
...
#ggsave("4_output/figs/supp_astrr.pdf",width = 6.0,height = 3.5)
...
#Figure 2 quintmat
f2_qm$yaxis_lab <- "Neighborhood material\ndeprivation quintile"
f2_qmp<-ggplot(f2_qm,aes(x = sex,y = factor(quintmat,levels = c(1:5)),
  ...
  geom_text(aes(label = rep_ratio,
                fontface = ifelse(rr_prob > 0.95,2,1)),
            color = "black",size = 3.2)+
  facet_grid(cols = vars(cohort), rows = vars(yaxis_lab))+
  labs(fill = "Representation Ratio")+
  theme(axis.title = element_blank(),
...
g2$heights[9] <- w* 4 #shrink bottom graph
g2$widths[13]<-2.68*g1$widths[5] #shrink width of mat dep part
gtable::gtable_show_layout(g2)
g2$widths[4]<-g2$widths[4] * 4.45 #shift bottom portion to the right while keeping axis label the same
g2$widths[5]<-g1$widths[5] * 1.22 #widen first panel
g2$widths[7]<-g1$widths[7] * 1.22 #widen second panel
g2$widths[9]<-g1$widths[9] * 1.22 #widen third panel
g2$widths[11]<-g1$widths[11] * 1.22 #widen fourth panel
g2$heights[9]<-g1$heights[7] * 7
f2_pf<-gridExtra::grid.arrange(g1,g2)

#ggsave("4_output/figs/f2_continuous.pdf",plot = f2_pf,width=8,height=10,unit="in")

# 4. Additional analyses: AARB and Duncan Dissimilarity Index -------------------------------------

# Plot AARB over time
aarb_df1 <- aarb_df %>% 
  mutate(sample_month = as.Date(sample_month))
colnames(aarb_df1)[2] <- "Study"

# Version 1
aarb_cols <- c("#65b6d0", "#85BC47","#D63D56",  "#7570B3","#D95F02")
aarb_month <- ggplot(data = aarb_df1[!is.na(aarb_df1$sample_month),], aes(x = sample_month, y = aarb_month, group = Study))+
  geom_point(aes(size = month_count_all, fill = Study), pch = 21, alpha = 0.75) +
  scale_size_continuous(name = "Monthly sample size",range = c(1,6))+ 
  scale_fill_manual(values = aarb_cols)+
  scale_color_manual(values = aarb_cols)+
  geom_line(aes(color = Study), linetype = "solid", alpha = 0.40)+ 
  scale_x_date(date_breaks = "2 months", date_labels = "%Y-%m")+
  scale_y_continuous(breaks = c(0, 20, 40, 60, 80, 100), limits = c(0,100))+
  labs(x = "Month of specimen collection",
       y = "Average absolute relative bias (%)") + 
  guides(size = "none")+
  coord_cartesian(xlim = c(as.Date("2020-05-01"), as.Date("2023-10-30")))

#ggsave("4_output/figs/aarb_month.pdf",plot = aarb_month, dpi = 6000, width = 15, height = 4, unit = "in")

# Plot overall DDI
ddi_df1 <- mutate(ddi_df,
                 variable = case_when(variable == "Race" ~ "Race/ethnicity",
                                      variable == "Quintmat" ~ "Neighborhood material\ndeprivation quintile",
                                      variable == "Quintsoc" ~ "Neighborhood social\ndeprivation quintile",
                                      .default = variable),
                 study = case_when(study == "cbs" ~ "CBS",
                                   study == "apl" ~ "APL",
                                   .default = study))

ddi_df1 <- mutate(ddi_df1,
                 variable = factor(variable, levels = c("Age", "Sex", "Urban", "Race/ethnicity", "Neighborhood material\ndeprivation quintile",
                                                        "Neighborhood social\ndeprivation quintile")),
                 study = factor(study, levels = c("Ab-C", "CanPath", "CLSA", "APL", "CBS")))

ddi_o <- ggplot(ddi_df1, aes(x = ddi, y = study))+
  geom_point() + 
  facet_grid(rows = vars(variable))+
  labs(y = "",
       x = "Duncan dissimilarity index")

#ggsave("4_output/figs/ddi_overall.pdf",plot = ddi_o, dpi = 1000, width = 10, height = 9.5, unit = "in")
...
#ggsave("4_output/figs/supp_sensr1.pdf",width=6.5,height=5.0,unit="in")
...
clsar1<-read.csv("./1_data/private/boot_clsa_asr1_5000.csv")
canr1<-read.csv("./1_data/private/boot_can_asr1_5000.csv")
...
boot_race1<-do.call("rbind",list(abcr1,clsar1,canr1,ccar1[,2:6]))
...
#ggsave("4_output/figs/supp_sensr1rr.pdf",width = 8, height = 6.0,units = "in")
...
#ggsave("4_output/figs/supp_sens2rr.pdf",plot = f2a2,width=7,height=9,unit="in")

# 7.Sensitivity analysis #3--------
# Age-sex-urban
clsa_dtasus3<-merge(asu_synth,clsa_asu1_qab,
                  by = c("age_groups","sex","urban"),all.x = TRUE)
abc_dtasus3<-merge(asu_synth,abc_asu1_qab,
                 by = c("age_groups","sex","urban"),all.x = TRUE)

#Calculate proportion each subgroup contributes to total dataset
clsa_dtasus3<-asu_clean(clsa_dtasus3)
abc_dtasus3<-asu_clean(abc_dtasus3)

#-- CLSA representation ratio --
clsapops3_count<-merge(clsa_dtasus3,g_asu,by = c("age_groups","sex","urban"),all.x = T)
colnames(clsapops3_count)[5]<-"pct_clsa" #percentage of total clsa samples in each subgroup
clsapops3_count<-asu_clean1(clsapops3_count)

#Calculate representation ratio -- value = 1 indicates cbs sample adequately represents the corresponding population subgroup
clsapops3_count$rep_ratio<-clsapops3_count$pct_clsa / clsapops3_count$pct_pop
clsapops3_count$cohort<-"CLSA\nquestionnaire"

#---Ab-C representation ratio---
abcpops3_count<-merge(abc_dtasus3,a_asu,by = c("age_groups","sex","urban"),all.x = T)
colnames(abcpops3_count)[5]<-"pct_abc" #percentage of total abc samples in each subgroup
abcpops3_count<-asu_clean1(abcpops3_count)

#Calculate representation ratio -- value = 1 indicates Ab-C sample adequately represents the corresponding population subgroup
abcpops3_count$rep_ratio<-abcpops3_count$pct_abc / abcpops3_count$pct_pop
abcpops3_count$cohort<-"Ab-C\nquestionnaire"

#Merge with main analysis df
allpopus3_count <- rbind(clsapop_count[,c("age_groups","sex","urban","rep_ratio","cohort")],
                         clsapops3_count[,c("age_groups","sex","urban","rep_ratio","cohort")],
                         abcpop_count[,c("age_groups","sex","urban","rep_ratio","cohort")],
                         abcpops3_count[,c("age_groups","sex","urban","rep_ratio","cohort")])

#Merge in bootstrap results and generate overall df
clsaus3 <- read.csv("./1_data/private/boot_clsa_asu_5000_qab.csv")
clsaus3$cohort <- "CLSA\nquestionnaire"
abcus3 <- read.csv("./1_data/private/boot_abc_asu_5000_qab.csv")
abcus3$cohort <- "Ab-C\nquestionnaire"
boot_urbans3 <- rbind(clsau,clsaus3, abcu, abcus3)

allpopus3_count <- merge(allpopus3_count,boot_urbans3, by = c("age_groups","sex","urban","cohort"),
                         all.x = T)

# Age-sex-race

#Calculate proportion each subgroup contributes to total dataset
clsa_dtasrs3<-merge(asr_synth[,c("age_groups","sex","race")],clsa_asr1_qab,
                  by = c("age_groups","sex","race"),all.x = TRUE)
clsa_dtasrs3$pct<-c(clsa_dtasrs3$count / sum(clsa_dtasrs3$count[1:24],na.rm = T))

abc_dtasrs3<-merge(asr_synth[,c("age_groups","sex","race")],abc_asr1_qab,
                 by = c("age_groups","sex","race"),all.x = TRUE)
abc_dtasrs3$pct<-c(abc_dtasrs3$count / sum(abc_dtasrs3$count[1:24],na.rm = T))

# --- CLSA representation ratio ---
clsapoprs3_count<-merge(clsa_dtasrs3,g_asr,by = c("age_groups","sex","race"),all.x = T)
colnames(clsapoprs3_count)[5]<-"pct_clsa" #percentage of total clsa samples in each subgroup
clsapoprs3_count<-clsapoprs3_count[order(clsapoprs3_count$age_groups,clsapoprs3_count$sex,
                                     clsapoprs3_count$race),]
clsapoprs3_count$pct_pop<-c(clsapoprs3_count$count_census/sum(clsapoprs3_count$count_census[1:24],na.rm = T))

#Calculate representation ratio -- value = 1 indicates clsa sample adequately represents the corresponding population subgroup
clsapoprs3_count$rep_ratio<-clsapoprs3_count$pct_clsa / clsapoprs3_count$pct_pop
clsapoprs3_count$cohort<-"CLSA\nquestionnaire"

# --- Ab-C representation ratio ---
abcpoprs3_count<-merge(abc_dtasrs3,a_asr,by = c("age_groups","sex","race"),all.x = T)
colnames(abcpoprs3_count)[5]<-"pct_abc"#percentage of total abc samples in each subgroup
abcpoprs_count<-abcpoprs3_count[order(abcpoprs3_count$age_groups,abcpoprs3_count$sex,
                                   abcpoprs3_count$race),]
abcpoprs3_count$pct_pop<-c(abcpoprs3_count$count_census/sum(abcpoprs3_count$count_census[1:24],na.rm = T))

#Calculate representation ratio -- value = 1 indicates ab-c sample adequately represents the corresponding population subgroup
abcpoprs3_count$rep_ratio<-abcpoprs3_count$pct_abc / abcpoprs3_count$pct_pop
abcpoprs3_count$cohort<-"Ab-C\nquestionnaire"

#Merge with main analysis df
allpoprs3_count <- rbind(clsapopr_count[,c("age_groups","sex","race","rep_ratio","cohort")],
                         clsapoprs3_count[,c("age_groups","sex","race","rep_ratio","cohort")],
                         abcpopr_count[,c("age_groups","sex","race","rep_ratio","cohort")],
                         abcpoprs3_count[,c("age_groups","sex","race","rep_ratio","cohort")])

#Merge in bootstrap results and generate overall df
clsars3<-read.csv("./1_data/private/boot_clsa_asr_5000_qab.csv")
clsars3$cohort<-"CLSA\nquestionnaire"
abcrs3 <- read.csv("./1_data/private/boot_abc_asr_5000_qab.csv")
abcrs3$cohort <- "Ab-C\nquestionnaire"
boot_races3<- rbind(clsar,clsars3,abcr,abcrs3)

allpoprs3_count<-merge(allpoprs3_count,boot_races3,by = c("age_groups","sex","race","cohort"),
                       all.x = T)

#Sex-quintmat

#Calculate proportion each subgroup contributes to total dataset
clsa_sqm1_qab$pct<-c(clsa_sqm1_qab$count / sum(clsa_sqm1_qab$count))

# --- CLSA representation ratio ---
clsapopqms3_count<-merge(clsa_sqm1_qab,g_sqm,by = c("sex","quintmat"),all.x = T)
colnames(clsapopqms3_count)[4]<-"pct_clsa" #percentage of total CLSA samples in each subgroup
clsapopqms3_count<-clsapopqms3_count[order(clsapopqms3_count$quintmat,clsapopqms3_count$sex),]
clsapopqms3_count$pct_pop<-c(clsapopqms3_count$count_census / sum(clsapopqms3_count$count_census))

#Calculate representation ratio -- value = 1 indicates clsa sample adequately represents the corresponding population subgroup
clsapopqms3_count$rep_ratio<-clsapopqms3_count$pct_clsa / clsapopqms3_count$pct_pop
clsapopqms3_count$cohort<-"CLSA\nquestionnaire"

#Merge with main analysis df
allpopqms3_count <- rbind(clsapopqm_count[,c("sex","quintmat","cohort","rep_ratio")],
                          clsapopqms3_count[,c("sex","quintmat","cohort","rep_ratio")])

#Merge in bootstrap results and generate overall df
clsaqms3 <- read.csv("./1_data/private/boot_clsa_sqm_5000_qab.csv")
clsaqms3$cohort <- "CLSA\nquestionnaire"
boot_qms3 <- rbind(clsaqm,clsaqms3)

allpopqms3_count <- merge(allpopqms3_count,boot_qms3,by = c("sex", "quintmat", "cohort"), all.x = T)

#Sex-quintsoc

#Calculate proportion each subgroup contributes to total dataset
clsa_sqs1_qab$pct<-c(clsa_sqs1_qab$count / sum(clsa_sqs1_qab$count))

# --- CLSA representation ratio ---
clsapopqss3_count<-merge(clsa_sqs1_qab,g_sqs,by = c("sex","quintsoc"),all.x = T)
colnames(clsapopqss3_count)[4]<-"pct_clsa" #percentage of total CLSA samples in each subgroup
clsapopqss3_count<-clsapopqss3_count[order(clsapopqss3_count$quintsoc,clsapopqss3_count$sex),]
clsapopqss3_count$pct_pop<-c(clsapopqss3_count$count_census / sum(clsapopqss3_count$count_census))

#Calculate representation ratio -- value = 1 indicates clsa sample adequately represents the corresponding population subgroup
clsapopqss3_count$rep_ratio<-clsapopqss3_count$pct_clsa / clsapopqss3_count$pct_pop
clsapopqss3_count$cohort<-"CLSA\nquestionnaire"

#Merge with main analysis df
allpopqss3_count <- rbind(clsapopqs_count[,c("sex","quintsoc","cohort","rep_ratio")],
                          clsapopqss3_count[,c("sex","quintsoc","cohort","rep_ratio")])

#Merge in bootstrap results and generate overall df
clsaqss3 <- read.csv("./1_data/private/boot_clsa_sqs_5000_qab.csv")
clsaqss3$cohort <- "CLSA\nquestionnaire"
boot_qss3 <- rbind(clsaqs,clsaqss3)

allpopqss3_count <- merge(allpopqss3_count,boot_qss3,by = c("sex","quintsoc", "cohort"),all.x = T)

# Generate plots
# Compose age-sex-urban and age-sex-race components of plot
colnames(allpopus3_count)[3]<-"strata"
colnames(allpoprs3_count)[3]<-"strata"
fs3<-rbind(allpopus3_count,allpoprs3_count)

fs3<-fs3 %>% mutate(
  cohort = case_when(
    cohort == "CLSA closed cohort" ~ "CLSA serology",
    cohort == "Ab-C open cohort" ~ "Ab-C serology",
    .default = cohort),
  strata = case_when(
    strata == "Racialized minority" ~ "Racialized\nminority",
    strata == "Urban" ~ "Urban",
...
fs3<-fs3 %>% mutate(
  strata = factor(strata,
                  levels = c("All","Rural","Urban","Racialized\nminority",
                             "White")),
  cohort=factor(cohort, levels = c("CLSA\nquestionnaire", "CLSA serology",
                                   "Ab-C\nquestionnaire", "Ab-C serology")),
  age_groups = factor(age_groups,
                      levels = c("All ages","0-17 years","18-26 years",
                                 "27-36 years","37-46 years","47-56 years",
                                 "57+ years"))
...
f1s3<-fs3 %>% filter(age_groups == "All ages")
f2s3<-fs3 %>% filter(age_groups != "All ages")

f2_qms3<-allpopqms3_count %>% 
  mutate(rep_ratio = round(as.numeric(rep_ratio),1),
         cohort = case_when(
           cohort == "CLSA closed cohort"~ "CLSA serology",
           .default = cohort))

f2_qss3<-allpopqss3_count %>% 
  mutate(rep_ratio = round(as.numeric(rep_ratio),1),
         cohort = case_when(
           cohort == "CLSA closed cohort"~ "CLSA serology",
           .default = cohort))
...
#Figure 1
f1_s3<-ggplot(f1s3,aes(x = sex,y = age_groups,
                    fill = rep_ratio))+
  geom_tile(color = "black",show.legend = T,linewidth = 0.1)+
  geom_text(aes(label = rep_ratio,
                fontface = ifelse(rr_prob > 0.95,2,1)), #changed 2nd arg from 2 to 1
  ...
  theme(axis.title = element_blank(),
        ...
        legend.text = element_text(size = 8.5),
        legend.margin = margin(t = 0.1,l = 0.1,r = 1.5))
f1_s3<-cowplot::plot_grid(f1_s3,scale = 0.75)
#ggsave("4_output/figs/f1s3.pdf",width=8.5,height=6.25,unit="in")


#Quintsoc
s1_sqss3<-ggplot(f2_qss3,aes(x = sex,y = factor(quintsoc,levels = c(1:5)),
                         fill = rep_ratio))+
  geom_tile(color = "black",show.legend = T,linewidth = 0.1)+
  geom_text(aes(label = rep_ratio,
                fontface = ifelse(rr_prob > 0.95,2,1)),
...
#ggsave("4_output/figs/s1_sqss3.pdf",width=7.0,height=4.0,unit="in")

#Figure 2
f2_s3<-ggplot(f2s3[f2s3$strata != "All",],aes(x = sex,y = age_groups,
                                              fill = rep_ratio))+
  geom_tile(color = "black",show.legend = T,linewidth = 0.1)+
  geom_text(aes(label = rep_ratio,
                fontface = ifelse(rr_prob > 0.95,2,1)),
...
#Figure 2 quintmat
f2_qms3$yaxis_lab <- "Neighborhood material\ndeprivation quintile"
f2_qms3p<-ggplot(f2_qms3,aes(x = sex,y = factor(quintmat,levels = c(1:5)),
                         fill = rep_ratio))+
  geom_tile(color = "black",show.legend = F,linewidth = 0.1)+
  scale_fill_gradientn(colours = cols,
                       values = c(rescale(x = c(0,7/10,1,10/7,3),to = c(0,1))),
...
gtable::gtable_show_layout(g2)
g2$widths[4]<-g2$widths[4] * 4.45 #shift bottom portion to the right while keeping axis label the same
g2$widths[5]<-g1$widths[5] * 0.90 #widen first panel
g2$widths[7]<-g1$widths[7] * 0.90 #widen second panel
#g2$widths[9]<-g1$widths[9] * 1.22 #widen third panel
#g2$widths[11]<-g1$widths[11] * 1.2 #widen fourth panel
g2$heights[9]<-g1$heights[7] * 7
gridExtra::grid.arrange(g1,g2)
f2_pf<-gridExtra::grid.arrange(g1,g2)

#ggsave("4_output/figs/f2s3_continuous1.pdf",plot = f2_pf,width=8,height=10,unit="in")


# 8. Sensitivity analysis 4 -----------------------
#Age-sex-urban
can_dtasus4<-merge(asu_synth,can_asu1_s4,
                 by = c("age_groups","sex","urban"),all.x = TRUE)

#Replace NA with 0 for subgroups that fall within eligibility criteria,
# but did not capture any participants in sample
can_dtasus4[can_dtasus4$age_groups == "18-26 years" &
            can_dtasus4$urban == "Rural","count"]<-0

#Calculate proportion each subgroup contributes to total dataset
can_dtasus4<-asu_clean(can_dtasus4)

#-- CanPath representation ratio --
canpops4_count<-merge(can_dtasus4,d_asu,by = c("age_groups","sex","urban"),all.x = T)
colnames(canpops4_count)[5]<-"pct_can" #percentage of total can samples in each subgroup
canpops4_count<-asu_clean1(canpops4_count)

#Calculate representation ratio -- value = 1 indicates CanPath sample adequately represents the corresponding population subgroup
canpops4_count$rep_ratio<-canpops4_count$pct_can / canpops4_count$pct_pop
canpops4_count$cohort<-"CanPath\nimputed province"

#Merge with main analysis df
allpopus4_count <- rbind(canpop_count,canpops4_count)
allpopus4_count <- allpopus4_count[,c("age_groups","sex","urban","rep_ratio","cohort")]

#Merge in bootstrap results and generate overall df
canus4 <- read.csv("./1_data/private/boot_can_asu_5000_s4.csv")
canus4$cohort <- "CanPath\nimputed province"
boot_urbans4 <- rbind(canu,canus4)

allpopus4_count <- merge(allpopus4_count,boot_urbans4, by = c("age_groups","sex","urban","cohort"),
                         all.x = T)

#Age-sex-race
can_dtasrs4<-merge(asr_synth[,c("age_groups","sex","race")],can_asr1_s4,
                 by = c("age_groups","sex","race"),all.x = TRUE)

#Replace NA with 0 for subgroups that fall within eligibility criteria,
# but did not capture any participants in sample
can_dtasrs4[can_dtasrs4$age_groups == "18-26 years" & 
            can_dtasrs4$sex == "Male" &
            can_dtasrs4$race == "White","count"]<-0

#Calculate proportion each subgroup contributes to total dataset
can_dtasrs4$pct<-c(can_dtasrs4$count / sum(can_dtasrs4$count[1:24],na.rm = T))

# --- CanPath representation ratio ---
canpoprs4_count<-merge(can_dtasrs4,d_asr,by = c("age_groups","sex","race"),all.x = T)
colnames(canpoprs4_count)[5]<-"pct_can"#percentage of total can samples in each subgroup
canpoprs4_count<-canpoprs4_count[order(canpoprs4_count$age_groups,canpoprs4_count$sex,
                                   canpoprs4_count$race),]
canpoprs4_count$pct_pop<-c(canpoprs4_count$count_census/sum(canpoprs4_count$count_census[1:24],na.rm = T))

#Calculate representation ratio -- value = 1 indicates CanPath sample adequately represents the corresponding population subgroup
canpoprs4_count$rep_ratio<-canpoprs4_count$pct_can / canpoprs4_count$pct_pop
canpoprs4_count$cohort<- "CanPath\nimputed province"

#Merge with main analysis df
allpoprs4_count <- rbind(canpopr_count,canpoprs4_count)
allpoprs4_count <- allpoprs4_count[,c("age_groups","sex","race","rep_ratio","cohort")]

#Merge in bootstrap results and generate overall df
canrs4<-read.csv("./1_data/private/boot_can_asr_5000_s4.csv")
canrs4$cohort<-"CanPath\nimputed province"
boot_races4<- rbind(canr,canrs4)

allpoprs4_count<-merge(allpoprs4_count,boot_races4,by = c("age_groups","sex","race","cohort"),
                       all.x = T)

# Generate plots
# Compose age-sex-urban and age-sex-race components of plot
colnames(allpopus4_count)[3]<-"strata"
colnames(allpoprs4_count)[3]<-"strata"
fs4<-rbind(allpopus4_count,allpoprs4_count)

fs4<-fs4 %>% mutate(
  cohort = ifelse(cohort == "CanPath closed cohort",
                  "CanPath without\nimputed province",
                  cohort),
  strata = case_when(
    strata == "Racialized minority" ~ "Racialized\nminority",
...
fs4<-fs4 %>% mutate(
  strata = factor(strata,
                  levels = c("All","Rural","Urban","Racialized\nminority",
                             "White")),
  cohort=factor(cohort, levels = c("CanPath without\nimputed province", "CanPath\nimputed province")),
  age_groups = factor(age_groups,
                      levels = c("All ages","0-17 years","18-26 years",
                                 "27-36 years","37-46 years","47-56 years",
                                 "57+ years"))
...
f1s4<-fs4 %>% filter(age_groups == "All ages")
f2s4<-fs4 %>% filter(age_groups != "All ages")

#Figure 1
f1_s4<-ggplot(f1s4,aes(x = sex,y = age_groups,
                       fill = rep_ratio))+
  geom_tile(color = "black",show.legend = T,linewidth = 0.1)+
  geom_text(aes(label = rep_ratio,
                fontface = ifelse(rr_prob > 0.95,2,1)), #changed 2nd arg from 2 to 1
  ...
  theme(axis.title = element_blank(),
        ...
        legend.text = element_text(size = 8.5),
        legend.margin = margin(t = 0.1,l = 0.1,r = 1.5))
f1_s4<-cowplot::plot_grid(f1_s4,scale = 0.75)
#ggsave("4_output/figs/f1s4.pdf",width=8.5,height=6.25,unit="in")

#Figure 2
f2_s4<-ggplot(f2s4[f2s4$strata != "All",],aes(x = sex,y = age_groups,
                                              fill = rep_ratio))+
  geom_tile(color = "black",show.legend = T,linewidth = 0.1)+
  geom_text(aes(label = rep_ratio,
                fontface = ifelse(rr_prob > 0.95,2,1)),
...
#ggsave("4_output/figs/f2_continuous1s4.pdf",plot = f2_s4,width=8,height=10,unit="in")
...
#ggsave("4_output/figs/boot_sur.tiff",plot = p_sur,width=11,height=9,unit="in")
...
p_sqm<-ggarrange(p_list[[1]],p_list[[2]],p_list[[3]],
          common.legend = T,legend = "bottom")
#ggsave("4_output/figs/boot_sqm.tiff",plot = p_sqm,width=8,height=6,unit="in")
...
p_sqs<-ggarrange(p_list[[1]],p_list[[2]],p_list[[3]],
                 common.legend = T,legend = "bottom")
#ggsave("4_output/figs/boot_sqs.tiff",plot = p_sqs,width=8,height=6,unit="in")
...
p_asu<-ggarrange(p_list[[1]],p_list[[2]],p_list[[3]],p_list[[4]],p_list[[5]],
                 common.legend = T,legend = "bottom")
#ggsave("4_output/figs/boot_asu.tiff",plot = p_asu,width=10,height=8,unit="in")
...
#ggsave("4_output/figs/boot_asr.tiff",plot = p_asr,width=10,height=8,unit="in")
...
p_asrm<-ggarrange(p_list[[1]],p_list[[2]],p_list[[3]],p_list[[5]],
                 common.legend = T,legend = "bottom")
#ggsave("4_output/figs/boot_asrm.tiff",plot = p_asrm,width=10,height=8,unit="in")
...
#ggsave("4_output/figs/boot_asw.tiff",plot = p_asw,width=10,height=8,unit="in")
